...
namespace NNAI
{
    // class BattleLSTM;
    class BattleNN;
    extern std::shared_ptr<NNAI::BattleNN> g_model1;
    extern std::shared_ptr<NNAI::BattleNN> g_model2;
    // Global model pointers for each color
    extern std::shared_ptr<BattleNN> g_model_blue;
    extern std::shared_ptr<BattleNN> g_model_green;
    extern std::shared_ptr<BattleNN> g_model_red;
    extern std::shared_ptr<BattleNN> g_model_yellow;
    extern std::shared_ptr<BattleNN> g_model_orange;
    extern std::shared_ptr<BattleNN> g_model_purple;
...
#include <torch/torch.h>
    ...
    struct BattleNNImpl : torch::nn::Module
    {
        // Backbone (feedforward NN)
        ...
        BattleNNImpl( int64_t input_size = 240, int64_t hidden_size = 128 )
            : backbone( torch::nn::Sequential( torch::nn::Linear( input_size, hidden_size ), torch::nn::ReLU(), torch::nn::Linear( hidden_size, hidden_size ),
                                               torch::nn::ReLU() ) )
            , action_type_head( hidden_size, 4 )
            , position_x_head( hidden_size, 9 )
            , position_y_head( hidden_size, 11 )
            ...
            , destination_y_head( hidden_size, 11 )
        {
            ...
            for ( auto & module : backbone->children() ) {
        ...
        std::vector<torch::Tensor> forward( torch::Tensor x )
        {
            ...
            // Output multiple heads
            torch::Tensor action_type_logits = action_type_head( features );
            torch::Tensor position_x_logits = position_x_head( features );
            torch::Tensor position_y_logits = position_y_head( features );
            torch::Tensor destination_x_logits = destination_x_head( features );
            torch::Tensor destination_y_logits = destination_y_head( features );

            return { action_type_logits, position_x_logits, position_y_logits, destination_x_logits, destination_y_logits };
        }
    };

    TORCH_MODULE( BattleNN );
