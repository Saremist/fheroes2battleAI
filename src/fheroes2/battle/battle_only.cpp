...
void FillRandomTrainingTroops( Heroes * hero, int monsterCount = -1 )
{
    if ( !hero )
        return;
    ...
    std::mt19937 gen( rd() );
    std::discrete_distribution<> dist( { 3, 6, 3 } ); // Centered around 3
    int numTroops = dist( gen ) + 2; // Gives 2 to 4, centered on 3

    if ( NNAI::isRunningExperiments && NNAI::isTraining )
        if ( monsterCount != -1 )
            numTroops = monsterCount;

    // int numTroops = 4; // Gives 2 to 5, centered on 3

    static const int troopIDs[5] = { 7, 45, 23, 34, 31 };
...
bool Battle::Only::setup( const bool allowBackup, bool & reset )
{
    ...
    // armyInfo[0].player.SetControl( armyInfo[0].controlType ); //OG Code
    armyInfo[0].player.SetControl( armyInfo[0].controlType );
    armyInfo[1].player.SetControl( armyInfo[1].controlType );
    ...
    for ( auto & info : armyInfo ) {
        if ( info.hero != nullptr ) {
            ...
            if ( NNAI::isRunningExperiments && NNAI::isTraining )
                FillRandomTrainingTroops( info.hero, info.monsterCount );
        }
        else {
